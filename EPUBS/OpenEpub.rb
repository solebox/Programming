...
class OpenEpub
	...
	def initialize(epubPath)
		@path = epubPath
		@entity_map = get_entity_map
	end
	# node_names - an array of the names of the nodes we want to mutate e.g: ["p","td"]
    # considering making this method private
	def iterate_epub_xhtml_dom(node_names,&block)
        xhtml_files = []
		...
		xhtml_files.each {|xhtml|
		    ...
		    node_names.each do |root| #O(k*n) //root types are finite (no idea what noko does, but i bet its O(nlogn) so complexity would be O(n*k*logn) 
	...
	# manipulate the raw html without using noko
    # considering making this method private
	def mutate_epub_xhtml_files_raw_html!(&block)
                xhtml_files = []
				Zip::ZipFile.foreach(@path) { |entry| 
			    if entry.to_s.match(XHTML_REGEX) then
			        xhtml_files << entry.to_s
	...
	def flip_nums(node_names)
		iterate_epub_xhtml_dom(node_names) do |node| 
			flip_nums_helper(node_names,node)
		end
	end

	
    # make this private
	def flip_nums_helper(node_names,node)
	    ...
	    node.children.each {|child|
	            flip_nums_helper(node_names,child) unless node_names.include?(child.name)
	    }
	end
    #considering making this one private
	def get_entity_map
		entity_map = {"&Aacute;"=>"&#193;", "&aacute;"=>"&#225;", "&Acirc;"=>"&#194;", "&acirc;"=>"&#226;", "&acute;"=>"&#180;", "&AElig;"=>"&#198;",
    ...
    def convert_entities
        mutate_epub_xhtml_files_raw_html! do |code| 
            code.gsub!(/\&\w{2,8};/,get_entity_map)
        end
    end
...
node_names = ["td","p","li"]
open_epub = OpenEpub.new(path)
open_epub.flip_nums(node_names)
open_epub.convert_entities
