...
class BisRape
    ...
    def rapeIt
        ...
        (1..CURRENT_PAGE_AMOUNT).each do |page|
            begin
                b = singlePageAttack(GET_PREFIX+page.to_s)
                puts GET_PREFIX+page.to_s+" is now being parsed..."
                if b.length != 0 then
                    cum << b
                end
            rescue
                puts "page number: "+page.to_s+" dropped."
            end
        end
...
rape = BisRape.new
puts rape.singlePageAttack(GET_PREFIX+82.to_s).empty?
#rape.rapeIt
